...
export default async function handler(req,res){
  ...
  if(!validName(patientName)||!validLast4(phoneLast4)||!validDate(p.originalDate)||!validTime(p.originalTime))return res.status(400).json({error:"예약 정보를 다시 확인해 주세요."});
  try{
    const {data}=await postToSheet({action:"cancel",patientName,phoneLast4,originalDate:String(p.originalDate),originalTime:String(p.originalTime)});
