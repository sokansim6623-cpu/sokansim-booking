...
async function loadAvailability(){const r=range();closedDates=new Set();bookedSlots=new Set();try{const response=await fetch(`/api/availability?start=${r.start}&end=${r.end}&_ts=${Date.now()}`,{cache:"no-store"}),result=await response.json();if(!response.ok)throw new Error(result.error);closedDates=new Set(result.closedDates||[]);bookedSlots=new Set(result.bookedSlots||[])}catch(e){showError(adminError,e.message||"예약 일정을 불러오지 못했습니다.")}renderCalendar()}
