...
const passwordCard=$("passwordCard"),passwordInput=$("adminPassword"),passwordButton=$("passwordButton"),passwordError=$("passwordError"),form=$("adminForm");
...
let adminPassword="",selectedTime="",closedDates=new Set(),bookedSlots=new Set(),calendarMonth=new Date();
...
function range(){const t=new Date(),min=t,max=addDays(t,90);return{min,max,start:toKey(min),end:toKey(max)}}
...
passwordButton.onclick=async()=>{
  clearError(passwordError);const value=passwordInput.value;
  if(!value)return showError(passwordError,"직원용 비밀번호를 입력해 주세요.");
  passwordButton.disabled=true;passwordButton.textContent="확인 중입니다…";
  try{
    const r=await fetch("/api/admin-check",{method:"POST",headers:{"x-admin-password":value}}),result=await r.json();
    if(!r.ok)throw new Error(result.error||"비밀번호를 확인해 주세요.");
    adminPassword=value;passwordCard.classList.add("hidden");form.classList.remove("hidden");await loadAvailability();form.scrollIntoView({behavior:"smooth"});
  }catch(e){showError(passwordError,e.message||"비밀번호를 확인해 주세요.")}
  finally{passwordButton.disabled=false;passwordButton.textContent="예약 등록 화면 열기"}
};
passwordInput.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();passwordButton.click()}});
...
form.onsubmit=async e=>{
  ...
  try{
    const response=await fetch("/api/admin-reservation",{method:"POST",headers:{"Content-Type":"application/json","x-admin-password":adminPassword},body:JSON.stringify({chartNo,patientName,phoneLast4,appointmentDate,appointmentTime:selectedTime,memo})}),result=await response.json();
